// High-performance, zero-dependency QR code matrix generator (Version 1-10, Error Correction M/L)
// Generates accurate standard QR code matrices offline in <2ms without external network requests
...
class QRCodeModel {
  ...
  addData(data) {
    this.dataList.push({
      ...
      data,
      getLength: () => (typeof data === 'string' ? new TextEncoder().encode(data).length : data.length),
      write: (buffer) => {
...
const QRUtil = {
  ...
  getLengthInBits: (mode, type) => {
    if (1 <= type && type < 10) {
      return mode === QRMode.MODE_NUMBER ? 10 : mode === QRMode.MODE_ALPHA_NUM ? 9 : 8
    } else if (type < 27) {
      return mode === QRMode.MODE_NUMBER ? 12 : mode === QRMode.MODE_ALPHA_NUM ? 11 : 16
    }
    return mode === QRMode.MODE_NUMBER ? 14 : mode === QRMode.MODE_ALPHA_NUM ? 13 : 16
  },
  getLostPoint: (qrCode) => {
...
/**
 * Draws a beautiful high-resolution QR code directly onto a Canvas element in <2ms
 */
export function drawQRCodeToCanvas(canvas, text, options = {}) {
  ...
  const qrDrawSize = width - margin * 2
  ...
  if (centerHeart) {
    ...
    const centerY = margin + qrDrawSize / 2
    const badgeRadius = Math.max(24, qrDrawSize * 0.1)
  ...
  if (customText && customText.trim()) {
    ...
    ctx.fillText(customText.trim(), width / 2, margin + qrDrawSize + 12)
  }
}
